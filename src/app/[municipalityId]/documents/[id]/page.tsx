...
interface Props {
  params: Promise<{ municipalityId: string; id: string }>;
}

export default async function DocumentDetailPage({ params }: Props) {
  const { municipalityId, id } = await params;
  const [doc, speakerBreakdown] = await Promise.all([
    getDocumentDetail(municipalityId, id),
    getDocumentSpeakerBreakdown(municipalityId, id),
  ]);
